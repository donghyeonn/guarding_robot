...
class WaypointIndexDictMain(Node):
    def __init__(self):
        ...
        self.normal_pub = self.create_publisher(String, '/robot2/waypoint_index', 10)
        self.active_sub = self.create_subscription(Bool, '/robot2/waypoint_active', self.active_callback, 10)
        self.robot2_index = self.create_subscription(String, '/robot3/waypoint_index', self.index_callback, 10)
        self.error_sub = self.create_subscription(Bool, '/cctv1/detect', self.error_callback, 10)
        self.thief_find3 = self.create_subscription(Float64MultiArray, 'robot2/dist_info',self.depth_callback, 10)        
