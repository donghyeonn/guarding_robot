...
class Patrol(LifecycleNode):
    def __init__(self):
        super().__init__('patrol_nav3_lifecycle')
    ...
    def on_configure(self, state: State) -> TransitionCallbackReturn:
        # 네비게이터 인스턴스 (robot3 네임스페이스)
        self.dock_navigator = TurtleBot4Navigator(namespace='/robot3')
        ...
        self.sub_cmd = self.create_subscription(String, '/robot3/waypoint_index', self.cb_command, 10)
        self.active = self.create_publisher(Bool, '/robot3/waypoint_active', 10)
        self.find = self.create_publisher(Bool, '/robot3/waypoint_find', 10)

        self.get_logger().info('Patrol 노드(robot3)가 configure 완료됨.')
        return TransitionCallbackReturn.SUCCESS

    def on_activate(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info('Patrol 노드(robot3)가 activate됨.')
        return TransitionCallbackReturn.SUCCESS

    def on_deactivate(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info('Patrol 노드(robot3)가 deactivate됨.')
        return TransitionCallbackReturn.SUCCESS
    ...
    def on_cleanup(self, state: State) -> TransitionCallbackReturn:
        ...
        self.active = None
        self.find = None
        self.get_logger().info('Patrol 노드(robot3)가 cleanup됨.')
        return TransitionCallbackReturn.SUCCESS

    def on_shutdown(self, state: State) -> TransitionCallbackReturn:
        self.get_logger().info('Patrol 노드(robot3)가 shutdown됨.')
        return TransitionCallbackReturn.SUCCESS
